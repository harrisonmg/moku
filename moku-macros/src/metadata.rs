...
impl Metadata {
    ...
    fn write_state_chart(&mut self) {
        // Simplified name: STATE_CHART instead of {NAME}_STATE_CHART
    ...
    fn write_state_enum(&mut self) {
        // Simplified name: State instead of {Name}State
    ...
    fn write_machine(&mut self) {
        // Simplified name: Machine instead of {Name}Machine
    ...
    /// Write the StateBuilder to the machine module.
    fn write_builder(&mut self) {
        // Simplified name: Builder instead of {Name}Builder
    ...
    fn write_states(&mut self) {
        ...
        self.top_state.for_each_state(|state, ancestors| {
            ...
            if !is_top_state {
                ...
                if state.autogen_enter {
                    imp.items.push(parse_quote! {
                        fn enter(_ctx: &mut Self::Context<'_>) -> ::moku::StateEntry<Self::State, Self> {
                ...
                statelike_impls.push(parse_quote! {
                    impl ::moku::internal::StateLike<#machine_mod::State, #event_local> for #state_ident {
                        ...
                        fn enter(ctx: &mut Self::Context<'_>) -> ::moku::StateEntry<#machine_mod::State, Self> {
                            <Self as ::moku::Substate<_>>::enter(ctx)
                        }
                        ...
                        ) -> impl Into<::moku::EventResponse<#machine_mod::State>> {
            ...
            // Context (renamed from Superstates)
            let context = state.context_ident();
