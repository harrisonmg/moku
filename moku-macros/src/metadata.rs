    ...
    fn write_top_state_impl(&mut self) {
        if let Some(mut imp) = self.top_state_impl.take() {
            ...
            imp.items.insert(
                2,
                parse_quote! {
                    type Next = ::moku::Next<Self::State>;
                },
            );
    ...
    fn write_states(&mut self) {
        ...
        self.top_state.for_each_state(|state, ancestors| {
            ...
            if !is_top_state {
                ...
                // Add the return type associated types
                imp.items.insert(
                    3,
                    parse_quote! {
                        type Entry = ::moku::Entry<Self::State, Self>;
                    },
                );
                imp.items.insert(
                    4,
                ...
                imp.items.insert(
                    5,
                    parse_quote! {
                        type Response = ::moku::Response<Self::State>;
                    },
                );
