...
pub fn build_metadata(name: Ident, module: ItemMod) -> Result<Metadata, syn::Error> {
    ...
    unpacker.validate_context_types()?;
...
```",
    ...
    /// Validate that the Context associated type is not manually defined for any Substate.
    fn validate_context_types(&self) -> Result<(), syn::Error> {
    ...
    fn unpack_substate(&mut self, imp: ItemImpl) {
        ...
        let superstate = match superstate {
            ...
            None => {
                self.error = Some(syn::Error::new(
                    last_segment.span(),
                    "the `moku::Substate` superstate must be a simple identifier, e.g. `impl Substate<Top> for Foo`",
                ));
                return;
            }
        };
        ...
        if self.states.iter().any(|s| s.ident == ident) {
            self.error = Some(syn::Error::new(
                imp.self_ty.span(),
                format!(
