...
/// Type containing mutable references to the parents of a [`TopState`], i.e. nothing.
///
/// Not intended for use by users.
pub struct NoParents<'a>(PhantomData<&'a ()>);
...
impl<T, U, V, W> From<Entry<T, V>> for NodeEntry<T, U, V, W>
        ...
        W: SubstateEnum<T, U, V>,
    {
        ...
        pub fn update(&mut self) {
            ...
            match self.node.update(&mut NoParents(PhantomData)) {
                Next::None => (),
                next => {
                    match next {
                    ...
                    while self.node.needs_update() {
                        match self.node.update_in_need(&mut NoParents(PhantomData)) {
                            Next::None => (),
                            Next::Target(target) => self.transition(target, true, false),
                            Next::ExactTarget(target) => self.transition(target, true, true),
                        }
        ...
        pub fn top_down_update(&mut self) {
            ...
            match self.node.top_down_update(&mut NoParents(PhantomData)) {
                Next::None => (),
                next => {
                    match next {
                        Next::None => unreachable!(),
                    ...
                    loop {
                        match self
                            .node
                            .top_down_update_in_need(&mut NoParents(PhantomData))
                        {
                            Next::None => break,
                            Next::Target(target) => self.transition(target, true, false),
                            Next::ExactTarget(target) => self.transition(target, true, true),
        ...
        pub fn transition_quiet(&mut self, target: T, indent: bool, exact: bool) {
            ...
            match self
                .node
                .transition(target, &mut NoParents(PhantomData), indent, exact)
            {
                TransitionResult::MoveUp => {
                    assert!(exact);
        ...
        pub fn handle_event(&mut self, event: &U) {
            ...
            match self.node.handle_event(&mut NoParents(PhantomData), event) {
                Response::Drop => (),
                Response::Next(next) => match next {
                    Next::None => (),
                    Next::Target(target) => self.transition(target, true, false),
